...
const Register = () => {
  ...
  return (
    // ✅ Scrollable wrapper so that full form is visible even on small screens
    <Box sx={{ height: '100%', overflowY: 'auto', py: 4 }}>
      <Container maxWidth="sm">
        <Paper elevation={6} sx={{ p: 4, borderRadius: 4 }}>
          <Typography variant="h5" align="center" sx={{ mb: 3, fontWeight: 700, color: '#2E7D32' }}>
            Member Registration
          </Typography>
          ...
          <form onSubmit={handleSubmit}>
            ...
            />
            <TextField
              fullWidth label="Re-enter Password" name="re_password"
              type={showRePassword ? 'text' : 'password'}
              margin="normal" value={form.re_password} onChange={handleChange} required
              InputProps={{
                endAdornment: (
                  <InputAdornment position="end">
            ...
            />
            <TextField
              fullWidth label="Secret Code" name="secret_code"
              margin="normal" value={form.secret_code} onChange={handleChange} required
              helperText="Enter the code provided by admin to register as member."
            />

            {/* ✅ Register Button – clearly visible now */}
            <Button
              type="submit"
              variant="contained"
              fullWidth
              sx={{ mt: 3, mb: 1, bgcolor: '#2E7D32', '&:hover': { bgcolor: '#1B5E20' } }}
            >
              Register
            </Button>
          </form>
