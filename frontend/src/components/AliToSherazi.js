...
function AliToSherazi({ user }) {
  ...
  const handleAddSubmit = async () => {
    ...
    if (!editData.parent_id) { alert('Parent ID is missing. Please click the "+" button again.'); return; }
    if (editData.generation_number === '' || isNaN(parseInt(editData.generation_number))) {
      alert('Generation number is required and must be a valid number'); return;
    }
  ...
  const handleEditSubmit = async () => {
    ...
    if (!editData.name.trim() || !editData.generation_number) {
      alert('Name and generation number are required'); return;
    }
    const fd = new FormData();
    fd.append('name', editData.name.trim());
    fd.append('generation_number', editData.generation_number);
  ...
  const handleDeleteNode = useCallback(async (nodeId, nodeName) => {
    if (!isAdmin) return;
    if (!window.confirm(`Are you sure you want to delete "${nodeName}"? This action cannot be undone.`)) return;
    try {
      await axios.delete(`/api/ali-sherazia/${nodeId}`, { headers: { 'x-auth-token': token } });
      loadChain();
    } catch (err) {
  ...
  const drawCombined = useCallback(() => {
    ...
    if (chain.length === 0) {
      ...
      svg.append('text').attr('x', '50%').attr('y', '50%').attr('text-anchor', 'middle')
        .attr('font-size', '24px').attr('fill', '#888').text('Add nodes using Admin Panel');
      return;
    }
    ...
    svg.call(zoom);
    svg.call(zoom.transform, d3.zoomIdentity.translate(50, 50).scale(0.9));

    const lineColor = '#2E7D32', lineWidth = 2.5;
    if (ali && hasan && hussain) {
      const aliBottomY = aliY + vBoxH, childTopY = childY;
      const midY = (aliBottomY + childTopY) / 2;
      const hasanCX = hasanX + vBoxW / 2, hussainCX = hussainX + vBoxW / 2;
      mainGroup.append('path').attr('fill', 'none').attr('stroke', lineColor).attr('stroke-width', lineWidth)
    ...
    const drawNode = (node, x, y, w, h, isAli = false) => {
      ...
      g.append('rect').attr('width', w).attr('height', h).attr('rx', 12).attr('ry', 12)
        .attr('fill', isAli ? '#fdf6e3' : '#ffffff')
        .attr('stroke', isAli ? '#D4AF37' : '#2E7D32')
        .attr('stroke-width', isAli ? 4 : 2.5);
      g.append('rect').attr('width', w).attr('height', isAli ? 8 : 6).attr('rx', 4).attr('ry', 4)
        .attr('fill', isAli ? '#D4AF37' : '#2E7D32');
      if (isAli) {
        g.append('text').attr('x', w - 30).attr('y', 30).attr('font-size', '24px').attr('text-anchor', 'middle').text('👑');
      }
      const father = node.father_name || '';
      let contentHtml = `<div style="font-weight:bold;">${node.name}</div>`;
      if (father) contentHtml += `<div>bin ${father}</div>`;
      if (node.wife_name) contentHtml += `<div>💍 ${node.wife_name}</div>`;
      contentHtml += `<div>Gen: ${node.generation_number}</div>`;
      if (node.info) contentHtml += `<div>${node.info}</div>`;
      const fontSize = calculateFontSize(contentHtml, w, h, isAli);
      g.append('foreignObject').attr('width', w - 40).attr('height', h - 80).attr('x', 20).attr('y', 20)
        .append('xhtml:div')
        .style('width', '100%').style('height', '100%')
        .style('display', 'flex').style('flex-direction', 'column')
        .style('align-items', 'center').style('justify-content', 'center')
        .style('text-align', 'center').style('font-size', `${fontSize}px`).style('line-height', '1.4')
        .html(contentHtml);

      if (isAdmin) {
        const deleteGroup = g.append('g').attr('transform', `translate(${w - 36}, 6)`).attr('cursor', 'pointer')
          .on('click', (e) => { e.stopPropagation(); handleDeleteNode(node.id, node.name); });
        deleteGroup.append('rect').attr('width', 28).attr('height', 20).attr('rx', 6)
          .attr('fill', '#fff').attr('stroke', '#d32f2f').attr('stroke-width', 1.5);
        deleteGroup.append('text').attr('x', 14).attr('y', 15).attr('text-anchor', 'middle')
          .attr('font-size', '14px').attr('font-weight', 'bold').attr('fill', '#d32f2f').text('✕');

        const addGroup = g.append('g').attr('transform', `translate(${w - 36}, ${h - 36})`).attr('cursor', 'pointer')
          .on('click', (e) => { e.stopPropagation(); handleAddChild(node.id); });
        addGroup.append('circle').attr('r', 14).attr('fill', '#2E7D32').attr('stroke', '#fff').attr('stroke-width', 2);
        addGroup.append('text').attr('x', 0).attr('y', 5).attr('text-anchor', 'middle')
          .attr('font-size', '16px').attr('fill', '#fff').attr('font-weight', 'bold').text('+');

        const editGroup = g.append('g').attr('transform', `translate(${w - 68}, ${h - 28})`).attr('cursor', 'pointer')
        ...
        editGroup.append('text').attr('x', 13).attr('y', 14).attr('text-anchor', 'middle')
          .attr('font-size', '12px').attr('font-weight', 'bold').attr('fill', '#1565C0').text('✎');
      }
    };
  ...
  useEffect(() => { if (chain.length > 0) drawCombined(); }, [chain, drawCombined]);
